...
const ResultCard = ({ result }) => {
  ...
  const handleShareToWhatsApp = () => {
    ...
    // Extract date from roomInfo
    const examDate = result.roomInfo.includes("DATE:")
      ? result.roomInfo.split("DATE:")[1].trim()
      : "N/A";
    ...
    const message = `*Exam Seat Details*\n\n` +
    ...
    <div className="mt-4 md:mt-6 bg-gradient-to-br from-[#18202e] to-[#0C0F1B] rounded-xl md:rounded-2xl shadow-2xl border border-[#666668]/20 p-4 sm:p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div className="bg-[#0C0F1B] p-3 sm:p-4 rounded-lg border border-[#666668]/20">
          ...
          <p className="font-semibold text-white text-base sm:text-lg break-all">{result.registerNumber}</p>
        </div>
        <div className="bg-[#0C0F1B] p-3 sm:p-4 rounded-lg border border-[#666668]/20">
          <p className="text-xs text-[#666668] uppercase font-medium mb-1">Seat No</p>
          <p className="font-semibold text-white text-base sm:text-lg">{result.seatNo}</p>
        </div>
        <div className="bg-[#0C0F1B] p-3 sm:p-4 rounded-lg border border-[#666668]/20">
